-- Function for super admin to directly create and approve university admin
-- This bypasses the normal application/approval process
-- The admin will complete university setup after logging in

CREATE OR REPLACE FUNCTION "public"."create_university_admin_by_super_admin"(
  p_super_admin_id uuid,
  p_auth_user_id uuid,
  p_email text,
  p_name text,
  p_phone text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_user_id uuid;
  v_result jsonb;
BEGIN
  -- Verify that the requester is a super admin
  IF NOT EXISTS (
    SELECT 1 FROM users
    WHERE id = p_super_admin_id
    AND role = 'super_admin'
    AND status = 'active'
  ) THEN
    RAISE EXCEPTION 'Only super admins can create university admins directly';
  END IF;

  -- Check if user already exists in users table
  IF EXISTS (SELECT 1 FROM users WHERE auth_user_id = p_auth_user_id) THEN
    RAISE EXCEPTION 'User already exists';
  END IF;

  -- Check if email already exists in users table
  IF EXISTS (SELECT 1 FROM users WHERE email = p_email) THEN
    RAISE EXCEPTION 'Email already exists';
  END IF;

  -- Create user record with auto-approval (no university assigned yet)
  INSERT INTO users (
    id,
    auth_user_id,
    email,
    name,
    phone,
    role,
    status,
    approval_status,
    university_id,
    approved_by,
    approval_date,
    application_date
  ) VALUES (
    gen_random_uuid(),
    p_auth_user_id,
    p_email,
    p_name,
    p_phone,
    'university_admin',
    'active',
    'approved',
    NULL,  -- No university yet - admin will create it after login
    p_super_admin_id,
    NOW(),
    NOW()
  )
  RETURNING id INTO v_user_id;

  -- Return result
  v_result := jsonb_build_object(
    'success', true,
    'user_id', v_user_id,
    'auth_user_id', p_auth_user_id,
    'message', 'University admin created successfully. They can now log in and complete university setup.'
  );

  RETURN v_result;

EXCEPTION
  WHEN OTHERS THEN
    RAISE EXCEPTION 'Error creating university admin: %', SQLERRM;
END;
$$;

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION "public"."create_university_admin_by_super_admin"(uuid, uuid, text, text, text) TO "authenticated";

-- Add comment
COMMENT ON FUNCTION "public"."create_university_admin_by_super_admin"(uuid, uuid, text, text, text) IS
'Allows super admins to directly create and auto-approve university admins. The admin will complete university setup after logging in.';
