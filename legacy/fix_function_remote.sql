...
-- This is necessary because multiple versions exist on the remote database

-- Drop the old version with (uuid, text, text, text, text, text, jsonb, text)
DROP FUNCTION IF EXISTS "public"."create_university_admin_by_super_admin"(uuid, text, text, text, text, text, jsonb, text);

-- Drop the new version with (uuid, uuid, text, text, text)
DROP FUNCTION IF EXISTS "public"."create_university_admin_by_super_admin"(uuid, uuid, text, text, text);

-- Drop any other potential versions
DROP FUNCTION IF EXISTS "public"."create_university_admin_by_super_admin"(text, text, text, uuid, text);
...
'Allows super admins to directly create and auto-approve university admins. Handles case where user is already created by handle_new_user trigger.';
