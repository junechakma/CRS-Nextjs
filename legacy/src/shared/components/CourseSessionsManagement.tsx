...
import { TeacherService, type ResponseSession, type Course, type CreateSessionData, type Question } from '../../features/teacher/services/teacherService'

interface CourseSessionsManagementProps {
  courseId: string
  course: Course
  readOnly?: boolean
}
...
const SessionModal: React.FC<SessionModalProps> = ({ isOpen, onClose, session, course, questions, durations, onSubmit, readOnly = false }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-4">
          {readOnly ? 'Session Details' : session ? 'Edit Response Session' : 'Create New Response Session'}
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => !readOnly && setFormData(prev => ({ ...prev, section: e.target.value }))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => !readOnly && setFormData(prev => ({ ...prev, session_date: e.target.value }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                required
                disabled={readOnly}
              />
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => !readOnly && setFormData(prev => ({ ...prev, duration_minutes: parseInt(e.target.value) }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                required
                disabled={readOnly}
              >
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => !readOnly && setFormData(prev => ({ ...prev, room_number: e.target.value }))}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
          ...
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2">
              ...
              {!readOnly && !session && questions.length > 0 && (
              ...
              {questions.length === 0 ? (
                ...
                <div className="space-y-2">
                  {questions.map(question => (
                    <label key={question.id} className={`flex items-start space-x-2 ${!readOnly ? 'cursor-pointer' : ''} p-2 rounded ${
                      ...
                      <input
                        ...
                        disabled={question.required || readOnly}
                        className={`mt-1 rounded border-gray-300 text-blue-600 focus:ring-blue-500 ${
                          question.required || readOnly ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                      />
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-1 sm:gap-2 text-xs text-gray-500 mt-1">
                          ...
                          {question.required && <span className="text-red-600 font-medium">Required</span>}
                        </div>
                      </div>
                    </label>
                  ))}
...
const CourseSessionsManagement: React.FC<CourseSessionsManagementProps> = ({ courseId, course, readOnly = false }) => {
  ...
  const loadData = async () => {
    ...
    try {
      setLoading(true)
      const [courseSessionsData, questionsData, durationsData] = await Promise.all([
        TeacherService.getCoursesSessions(courseId),
      ...
      const updatedSessions = await Promise.all(
        courseSessionsData.map(async (session) => {
          if (session.status === 'active') {
            const endTime = new Date(session.end_time)
            if (now > endTime) {
  ...
  const handleViewAnalytics = (session: ResponseSession) => {
    ...
    switch (user.role) {
      case 'teacher':
        analyticsPath = `/teacher/sessions/${session.id}/analytics`
        break
      case 'university_admin':
        analyticsPath = `/university-admin/sessions/${session.id}/analytics`
        break
      case 'faculty_admin':
        analyticsPath = `/faculty-admin/sessions/${session.id}/analytics`
        break
      case 'department_moderator':
        analyticsPath = `/department-moderator/sessions/${session.id}/analytics`
        break
      default:
        console.warn('Unknown role for analytics navigation:', user.role)
        return
    }
  ...
  const filteredSessions = sessions.filter(session => {
    const matchesSearch = session.section.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (session.room_number && session.room_number.toLowerCase().includes(searchTerm.toLowerCase()))
  ...
  const SessionCard: React.FC<{
    ...
    readOnly: boolean
  }> = ({ session, onEdit, onDelete, onStatusChange, onViewAnalytics, readOnly }) => {
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <div>
                <h3 className="font-medium text-gray-900">Section {session.section}</h3>
                <p className="text-sm text-gray-600">{formatDate(session.session_date)}</p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">
                {session.room_number || 'No room specified'}
              </p>
            </div>
          </div>
          <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">{session.stats.total_responses}</p>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6">
      <Card title={`Course Sessions (${filteredSessions.length})`}>
...
export default CourseSessionsManagement