...
export default function DepartmentManagement() {
  ...
  const handleCreateDepartment = async () => {
    ...
    try {
      ...
      if (formData.assign_moderator) {
        ...
        result = await UniversityAdminService.createDepartmentWithModerator(
          user.id,
          {
            ...formData,
        ...
        if (result.success) {
          ...
          resetForm()
          alert(`Department and Moderator created successfully!\n\nModerator Email: ${formData.moderator_email}\nTemporary Password: ${tempPassword}\n\nThe moderator can log in with this password and will be required to change it on first login.\n\nIMPORTANT: Please share this password with the moderator securely.`)
        }
      } else {
        // Create department without moderator
        result = await UniversityAdminService.createDepartment(
          user.id,
          {
            name: formData.name,
            code: formData.code,
  ...
  const handleEditDepartment = async () => {
    ...
    try {
      ...
      await UniversityAdminService.updateDepartment(selectedDepartment.id, {
        name: formData.name,
        code: formData.code,
        description: formData.description,
        faculty_id: formData.faculty_id
  ...
  const handleDeleteDepartment = async (departmentId: string) => {
    if (!confirm('Are you sure you want to delete this department? This will also delete all teachers, courses and associated data.')) {
      return
    }
  ...
  const handleAssignModerator = async () => {
    ...
    try {
      ...
      const result = await UniversityAdminService.assignDepartmentModerator(
      ...
      if (result.success) {
        ...
        setSelectedDepartment(null)
        const assignedEmail = moderatorFormData.moderator_email
        const assignedPassword = moderatorFormData.temp_password
        setModeratorFormData({
          moderator_name: '',
          moderator_email: '',
          moderator_phone: '',
        ...
        alert(`Department moderator assigned successfully!\n\nEmail: ${assignedEmail}\nTemporary Password: ${assignedPassword}\n\nThe moderator can log in with this password and will be required to change it on first login.\n\nIMPORTANT: Please share this password with the moderator securely.`)
      } else {
        alert(`Error: ${result.error}`)
      }
    } catch (error) {
  ...
  const loadDepartmentModerators = async () => {
    try {
      ...
      const moderatorsData = await UniversityAdminService.getDepartmentModerators(user.university_id)
  ...
  const handleAddNewModerator = async () => {
    if (!user?.id || !user?.university_id) return

    try {
      setFormLoading(true)
      
      const result = await UniversityAdminService.addDepartmentModerator(
        user.university_id,
        moderatorFormData
      )
      ...
      if (result.success) {
        await loadDepartmentModerators()
        const addedEmail = moderatorFormData.moderator_email
        const addedPassword = moderatorFormData.temp_password
        setModeratorFormData({
          moderator_name: '',
          moderator_email: '',
          moderator_phone: '',
        ...
        alert(`Department moderator added successfully!\n\nEmail: ${addedEmail}\nTemporary Password: ${addedPassword}\n\nThe moderator can log in with this password and will be required to change it on first login.\n\nIMPORTANT: Please share this password with the moderator securely.`)
      } else {
        alert(`Error: ${result.error}`)
      }
    } catch (error) {
      console.error('Error adding department moderator:', error)
      alert('Failed to add department moderator')
    } finally {
      setFormLoading(false)
    }
  }
  ...
  const handleRemoveModerator = async (moderatorId: string) => {
    ...
    try {
      await UniversityAdminService.removeDepartmentModerator(moderatorId)
      await loadDepartmentModerators()
  ...
  const filteredDepartments = departments.filter(department => {
    const matchesSearch = department.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      department.faculty_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (department.moderator_name && department.moderator_name.toLowerCase().includes(searchTerm.toLowerCase()))
    
    const matchesFaculty = !selectedFaculty || department.faculty_id === selectedFaculty
        ...
        <div className="space-y-6">
          ...
          <div className="border rounded-lg p-4 bg-gray-50">
            ...
            <div className="flex justify-end mt-4">
              <Button onClick={handleAddNewModerator} loading={formLoading}>
                <UserPlus className="w-4 h-4 mr-2" />
                Add Moderator
              </Button>
            </div>
                  ...
                  <div key={moderator.id} className="flex items-center justify-between p-4 border rounded-lg bg-white">
                    <div className="flex-1">
                      ...
                      {moderator.department_name && (
                        <div className="text-sm text-blue-600">Department: {moderator.department_name}</div>
                      )}
