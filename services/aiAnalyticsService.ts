...
- Distribution: ${JSON.stringify(q.distribution)}`
        } else if (q.type === "boolean" && q.distribution) {
          questionSummary += `
- Distribution: ${JSON.stringify(q.distribution)}`
        } else if (q.type === "text") {
          questionSummary += `
- Sample Responses: ${q.responses.slice(0, 5).join("; ")}`
        } else if (q.type === "multiple" && q.distribution) {
          questionSummary += `
- Distribution: ${JSON.stringify(q.distribution)}`
        } else if (q.type === "numeric" && q.averageRating !== undefined) {
          questionSummary += `
- Average: ${q.averageRating}
- Distribution: ${JSON.stringify(q.distribution)}`
        }
...
SESSION INFORMATION:
Course: ${sessionInfo.courseCode} - ${sessionInfo.courseTitle}${sessionInfo.section ? ` (Section ${sessionInfo.section})` : ""}
